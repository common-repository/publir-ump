...
class PublirSettingsPage
{
    ...
    {   
        ...
        add_settings_field(
            'publir_subs_programming', // ID
            'Subscription Programming:', // Title 
            array( $this, 'publir_publir_subs_programming_callback' ), // Callback
            'pb-admin', // Page
        ...
        add_settings_field(
            'publir_secret_key', // ID
            'Secret key:', // Title 
            array( $this, 'publir_secret_key_callback' ), // Callback
            'pb-admin', // Page
            'setting_page_id' // Section           
        ); 
    ...
    */
    ...
    public function sanitize( $input )
    {
        ...
        if( isset( $input['publir_secret_key'] ) )
            $new_input['publir_secret_key'] = sanitize_text_field( $input['publir_secret_key'] );
        
        if( isset( $input['publir_subs_programming'] ) )
            $new_input['publir_subs_programming'] = sanitize_text_field( $input['publir_subs_programming'] );
        ...
        $pathToMksFile = plugin_dir_path( __FILE__ ) . 'assets/js/mks.js';
        $contentMksFile = file_get_contents($pathToMksFile);
        $upload = wp_upload_dir();
        $filename = sanitize_text_field( $input['publir_site_id']).'.js';

        if(file_exists($upload['path'].'/'.$filename)) unlink($upload['path'].'/'.$filename);
    ...
    public function publir_secret_key_callback(){
        $secret_key = isset($this->options['publir_secret_key']) && $this->options['publir_secret_key'] !== '' ? esc_attr($this->options['publir_secret_key']) : 'PSK'.$this->generate_random_code();
        printf(
            '<input type="text" id="publir_secret_key" name="publir_wp_options[publir_secret_key]" readonly autocomplete="off" value="%s" required />',
            $secret_key
        );
    }

    public function publir_publir_subs_programming_callback()
    {
        printf(
            '<input type="text" id="publir_subs_programming" name="publir_wp_options[publir_subs_programming]" value="%s" required autocomplete="off" />',
            isset( $this->options['publir_subs_programming'] ) ? esc_attr( $this->options['publir_subs_programming']) : 'Ad-Free'
        );
    }
    ...
    public function generate_random_code($length = 15) {
