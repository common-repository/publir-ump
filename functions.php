	...
	*/
	function publir_enqueue_script() {
		...
		if($option){
			...
			wp_enqueue_script( 'publir-script', $src , array(), date('his'), false );
	...
	*/
	function publir_async_scripts($url)
	{
		...
		if(!empty($options)){
			...
			if ( strpos( $url, 'uploads/'.$site_id) == true || strpos( $url, 'platform/'.$site_id) == true ){
				return str_replace( ' src', ' async data-cfasync="false" src', $url );
			}else{
				return $url;
			}
		}
...
add_filter( 'script_loader_tag', 'publir_async_scripts', 10, 3 );
